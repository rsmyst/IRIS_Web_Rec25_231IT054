...
export default function ReviewCourtBookingRequests() {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        setLoading(true);
        const response = await axios.get("/api/admin/court-bookings");
        setBookings(response.data);
        setLoading(false);
      } catch (err) {
        console.error("Error fetching court bookings:", err);
        setError("Failed to load court bookings. Please try again later.");
        setLoading(false);
      }
    };
  ...
  const updateBookingStatus = async (bookingId: string, status: string) => {
    try {
      await axios.put(`/api/admin/court-bookings/${bookingId}`, { status });
      setBookings(
    ...
    <div className="bg-gray-900 text-gray-100 min-h-screen p-6 rounded-lg">
      ...
      {loading ? (
        ...
        <div className="bg-gray-800 p-6 rounded-lg shadow-md text-center border border-gray-700">
          <p className="text-gray-400">No court booking requests found</p>
        </div>
      ) : (
